...
#define FIRMWARE_VERSION_INFO F("+v1.0")  
#define FIRMWARE_VERSION_NUM 0x01       //1st Complete Project : 1 
...
/**
 * The OpenVFO is an upconversion transceiver. The first IF is at 45 MHz.
...
byte advancedFreqOption1;     //255 : Bit0: use IFTune_Value, Bit1 : use Stored enabled SDR Mode, Bit2~Bit3 : dynamic sdr frequency,  bit 7: IFTune_Value Reverse for DIY OpenVFO
...
byte I2C_LCD_MASTER_ADDRESS;        //0x27  //if Set I2C Address by OpenVFO Manager, read from EEProm
...
/**
 * Below are the basic functions that control the openvfo. Understanding the functions before 
...
void setFrequency(unsigned long f){
  ...
  //applied if tune 
  //byte advancedFreqOption1;     //255 : Bit0: use IFTune_Value, Bit1 : use Stored enabled SDR Mode, Bit2 : dynamic sdr frequency0, Bit3 : dynamic sdr frequency1, bit 7: IFTune_Value Reverse for DIY OpenVFO
  ...
  //for DIY OpenVFO (custom filter)
  if ((advancedFreqOption1 & 0x80) != 0x00)  //Reverse IF Tune (- Value for DIY OpenVFO)
    if1AdjustValue *= -1;
    
  if (sdrModeOn && (inTx == 0))  //IF SDR MODE
  {
    //Fixed Frequency SDR (Default Frequency : 32Mhz, available change sdr Frequency by OpenVFO Manager)
...
/*
 save Frequency and mode to eeprom for Auto Save with protected eeprom cycle
 */
...
//calculate step size from 1 byte, compatible OpenVFO Manager, by OPENVFO-Consortium
...
void initSettings(){
  ...
  //----------------------------------------------------------------
  //Add Lines by OPENVFO-Consortium
...
//    hamBandRange[10][0] = 50000; hamBandRange[9][1] = 54000;
//  }
...
void initPorts(){
  ...
  pinMode(ANALOG_SMETER, INPUT); //by OPENVFO-Consortium
...
//Auto save Frequency and Mode with Protected eeprom life by OPENVFO-Consortium
